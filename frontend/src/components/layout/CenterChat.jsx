...
const CenterChat = () => {
  ...
  const loadMessages = async ({ conversationId, beforeSequence = null, append = false, newerOnly = false } = {}) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (container.scrollTop <= 80 && !loadingMore && hasMore && nextBeforeSequence != null) {
        setLoadingMore(true);
        loadMessages({
  ...
  }, [activeConversation?.id, hasMore, loadingMore, nextBeforeSequence]);
